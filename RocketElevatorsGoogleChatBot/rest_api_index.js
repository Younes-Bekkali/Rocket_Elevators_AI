...
exports.dialogflowFirebaseFulfillment = functions.https.onRequest((request, response) => {
    ...
    function RocketElevators(agent){
        ...
        .then((result) => {
            ...
            // agent.add('There are currently ' + nb_elevators + ' elevators deployed.');
        })
        .then(() => {
            ...
            .then(() => {
                return axios.get('https://rocketelevatorsrestapi.herokuapp.com/api/Buildings')
                .then((result) => {
                    console.log(result.data);
                    response.nb_buildings = result.data.length;
                })
                .then(() => {
                    return axios.get('https://rocketelevatorsrestapi.herokuapp.com/api/Customers')
                    .then((result) => {
                        console.log(result.data);
                        response.nb_customers = result.data.length;
                    })
                    .then(() => {
                        return axios.get('https://rocketelevatorsrestapi.herokuapp.com/api/Leads')
                        .then((result) => {
                            console.log(result.data);
                            response.nb_leads = result.data.length;
                        })
                        .then(() => {
                            return axios.get('https://rocketelevatorsrestapi.herokuapp.com/api/Quotes')
                            .then((result) => {
                                console.log(result.data);
                                response.nb_quotes = result.data.length;
                            })
                            .then(() => {
                                return axios.get('https://rocketelevatorsrestapi.herokuapp.com/api/Batteries')
                                .then((result) => {
                                    console.log(result.data);
                                    response.nb_batteries = result.data.length;
                                })
                                .then(() => {
                                    return axios.get('https://rocketelevatorsrestapi.herokuapp.com/api/Addresses/cities')
                                    .then((result) => {
                                        console.log(result.data);
